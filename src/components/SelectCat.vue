                    ...
                    <div class="cat-set-middle">
                        <template v-for="(cat, index) in catset.catlist">
                            <v-chip :key="index" label outline color="blue">
                                <input type="checkbox" v-model="selectedCats" :value="cat.name" @change="boxChanged(cat.name, $event, false)" />&nbsp;<span>{{ cat.name }}</span>
                            </v-chip>
                        </template>
                    </div>
                    <div class="cat-set">
                        <span v-if="catset.parent !== ''"><v-icon @click="upClicked(catset.parent)">arrow_drop_up</v-icon><span class="parent-text">{{ catset.parent }}</span></span>
                        <br>
                        <span v-if="checkDownArraw(catset)"><v-icon @click="downClicked(catset)">arrow_drop_down</v-icon></span>&nbsp;
                    </div>
                </div>
            </v-list-tile-content>
        </v-list-tile>
...
export default {
    ...
    methods: {
        ...
        boxChanged(category, e, simulate) {
            ...
            BUS.session.barInfo = "Selected Categorie(s): " + this.selectedCats.join(", ");
            BUS.updateSession();
        },
        ...
        checkDownArraw(categorySet) {
            for (let i = 0; i < categorySet.catlist.length; i++) {
                if (this.selectedCats.indexOf(categorySet.catlist[i].name) >= 0) {
                    if (categorySet.catlist[i].children !== null && categorySet.catlist[i].children.length > 0) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    data() {
        return {
            ...
            catsetlist: [], // to control category sets that displayed on the page
    ...
    created() {
        ...
        query1.containsAll("parents", ["INDIVIDUAL", "ENTITY", "SELFMOVING", "NONSTATIONARY", "PHYSICAL", "REAL", "LIVING", "NATURAL"]);
        ...
        query2.containedBy("parents", ["INDIVIDUAL", "ENTITY", "SELFMOVING", "NONSTATIONARY", "PHYSICAL", "REAL", "LIVING", "NATURAL"]);
